...
> The backend persists links in SQLite, so they now survive restarts and
> redeploys (as long as `DB_PATH` is on a mounted volume) — the tests still
> always create fresh short links rather than depending on any code from an
> earlier run, but this also means every load test run adds to the link
> count permanently rather than starting from empty. That makes the
> "unbounded list growth" behavior noted under Stress and Soak below worse
> over successive runs, since `GET /api/links` has no pagination.
